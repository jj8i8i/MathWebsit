// ===== SCRIPT.JS (FINAL SIMPLIFIED & ROBUST) =====
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showScreen = (screenId) => {
        ...
        // Re-render math on the newly shown screen
        renderMath();
    };
    ...
    const renderMath = () => {
        ...
        if (window.renderMathInElement) {
            try {
             ...
             console.warn("KaTeX not loaded yet, skipping math rendering."); // Debug log
        }
    };
    ...
    const initApp = () => {
        ...
        if (savedUser && credentials[savedUser]) {
            console.log(`Found logged in user: ${savedUser}`); // Debug log
            currentUser = savedUser;
            showScreen('selection');
            renderSelectionScreen();
        } else {
            console.log("No logged in user found, showing login."); // Debug log
            showScreen('login');
        }
    ...
    // --- Start the App ---
    // No need to wait for KaTeX explicitly, renderMath checks internally
    initApp();
...
const updateWelcomeMessage = () => { welcomeMessages.forEach(msg => { if (currentUser) { msg.textContent = currentLang === 'en' ? `Welcome, ${currentUser}` : `ยินดีต้อนรับ, ${currentUser}`; } }); };
...
const logoutAction = () => { currentUser = null; localStorage.removeItem('loggedInUser'); loginStuff.usernameInput.value = ''; loginStuff.passwordInput.value = ''; showScreen('login'); };
