// ===== SCRIPT.JS (FINAL CORRECTED INITIALIZATION) =====
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Data (Full data is placed within the script now) ---
    ...
    const allSolutions = {
        'nt1': {
            en: [
                ...
                { answer: "6", steps: "We need $n | 2^n+2$. If $n$ is even, let $n=2k$. The condition becomes $k | (2^{2k-1}+1)$.\n\nFor this to hold, $k$ must be odd (unless $k=1$).\n\nFor $k=1$, $1|2^1+1$ is true, giving $n=2$. This is the smallest solution not a power of 3.\n\nFor $k=3$, $3|2^5+1=33$ is true, giving $n=6$. This is the second smallest solution not a power of 3." },
                { answer: "8", steps: "This is the Markov equation. We find solutions using Vieta Jumping. Start with the base solution $(1,1,1)$.\n\nFix $(x,y)=(1,1)$, and solve the resulting quadratic $t^2 - 3xyt + (x^2+y^2)=0$. This gives $t^2-3t+2=0$, with roots $t=1,2$, generating $(1,1,2)$.\n\nNext, starting with $(1,2)$, the quadratic is $t^2 - 6t+5=0$, with roots $t=1,5$, generating $(1,2,5)$.\n\nThe largest solution with $z<10$ is $(1,2,5)$. The sum is $1+2+5=8$." },
                { answer: "45", steps: "The expression $\\lfloor \\sqrt{n} + \\frac{1}{2} \\rfloor$ is rounding $\\sqrt{n}$ to the nearest integer. Let $k = \\text{round}(\\sqrt{n})$. This holds for $n$ in the range $k^2-k+1 \\le n \\le k^2+k$.\n\nFor such $n$, $a_n = n+k$. The set of values for a fixed $k$ is the range $[k^2+1, k^2+2k]$.\n\nThe integers missing from the sequence are the perfect squares: $1, 4, 9, \\dots$. We need the number of perfect squares $\\le 2025$. Since $\\sqrt{2025} = 45$, there are 45 such numbers." },
                { answer: "-1", steps: "The product is $P = (-1)^{h_{(-p)}}$, where $h_{(-p)}$ is the class number of the imaginary quadratic field $\\mathbb{Q}(\\sqrt{-p})$.\n\nFor a prime $p>3$ with $p \\equiv 3 \\pmod 4$, the class number $h_{(-p)}$ is odd if and only if $p \\equiv 3 \\pmod 8$.\n\nWe check $2027 \\equiv 3 \\pmod 8$. Since the class number is odd, $P = (-1)^{\\text{odd}} = -1$." },
                { answer: "999", steps: "The equation $x^2 - Dy^2 = -1$ has a solution if and only if the period length of the continued fraction of $\\sqrt{D}$ is odd.\n\nHere, $D = n^2+1$. The continued fraction for $\\sqrt{n^2+1}$ is $[n; \\overline{2n}]$. The period length is 1.\n\nSince 1 is odd, the equation has a solution for every positive integer $n$. The largest $n < 1000$ is 999." },
    ...
    const initApp = () => {
        ...
        Object.keys(allSolutions).forEach(setId => {
            if (allSolutions[setId] && allSolutions[setId].th) {
                allSolutions[setId].th.forEach((sol_th, i) => {
                    if (allSolutions[setId].en && allSolutions[setId].en[i]) {
                        sol_th.answer = allSolutions[setId].en[i].answer;
                    }
                    if (allProblems[setId] && allProblems[setId].th && allProblems[setId].th[i]) {
